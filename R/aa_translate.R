...
aa_translate = function(x, to){
  ...
  # Detect input type
  input_type = NULL
  if( all(x[!is.na(x)] %in% AMINOACIDS$one) ){
    input_type = 'one'
  } else {
    if( all(x[!is.na(x)] %in% AMINOACIDS$three) ){
      input_type = 'three'
    } else {
      if( all(x[!is.na(x)] %in% AMINOACIDS$full) ){
        input_type = 'full'
      }
    }
  }
  ...
  # Do translation
  translation_table = AMINOACIDS
  rownames(translation_table) = AMINOACIDS[,input_type]
