...
#' by chance. These values are then divided by 5 to accomodate scaled input,
#' to neural networks. \code{'X'} is encoded as the null-vector.
#'
...
#' @param mat An encoding matrix, one of c("BLOSUM50", "BLOSUM62", "BLOSUM50_pca", "BLOSUM62_pca")
...
#' dim(X6)
#'
#' custom_mat = round(matrix(data = rnorm(n = 441), nrow = 21, ncol = 21, dimnames = list(c(AMINOACIDS$one, 'X'), c(AMINOACIDS$one, 'X'))), 2)
#' X7 = pep_encode(peps, mat = custom_mat)
#' dim(X7)
#'
#' @export
pep_encode = function(pep, mat, n_pcs = 10){
  ...
  # Check custom matrix
  if( !(mat %in% m_allowed) && !is.numeric(mat) ){
